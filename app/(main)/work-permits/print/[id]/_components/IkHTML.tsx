// ==========================================
// KOMPONEN HTML: HALAMAN IK (INSTRUKSI KERJA)
// ==========================================
...
// ── Tanda tangan ─────────────────────────────────────────────────────────────
const IkSignaturesHTML = ({ permit }: { permit: Permit }) => (
  <div className="mt-8">
...
// ── Main Component ────────────────────────────────────────────────────────────
export const IkHTML = ({ permit }: Props) => {
  ...
  return (
    <div className="bg-white p-10 shadow-lg break-before-page print:m-0 print:max-w-none print:p-0 print:shadow-none">
      <div className="border-2 border-black text-xs text-black">
        ...
        {ikDocs.length === 0 ? (
          <p className="p-6 text-center italic text-gray-400">
            Tidak ada data IK terlampir.
          </p>
        ) : (
          ikDocs.map((ik, idx) => (
            <div key={idx}>
              {/* Judul IK jika lebih dari satu */}
              {ikDocs.length > 1 && (
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="px-4 py-3">
                      <EquipmentGrid items={ik.perlengkapanKerja ?? []} />
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <table className="w-full border-b border-black">
                <tbody>
                  <tr>
                    ...
                    <td className="px-4 py-3">
                      <EquipmentGrid items={ik.peralatanUkur ?? []} />
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <table className="w-full border-b border-black">
                <tbody>
                  <tr>
                    ...
                    <td className="px-4 py-3">
                      <EquipmentGrid items={ik.peralatanKerja ?? []} />
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <table className="w-full">
                <tbody>
                  <tr>
                    ...
                    <td className="px-4 py-3">
                      {/* ✅ judulUraianKegiatan dicetak TEBAL, uraianKegiatan di bawahnya normal */}
                      {(ik.judulUraianKegiatan ?? []).length > 0 ? (
                        <div className="space-y-0.5">
                          {(ik.judulUraianKegiatan ?? []).map((judul, i) => {
                            // Hitung range uraian untuk judul ini
                            const nextJudulIdx = (
                              ik.judulUraianKegiatan ?? []
                            ).findIndex((_, ji) => ji > i);
                            const uraianSlice = (ik.uraianKegiatan ?? []).slice(
                              i,
                        ...
                        // Fallback: langsung render uraianKegiatan dengan deteksi judul otomatis
                        <div className="space-y-0.5">
                          {(ik.uraianKegiatan ?? []).map((uraian, i) => {
                            ...
                            return (
                              <p
                                ...
                                className={`text-xs leading-relaxed ${
                                  isJudul ? "font-bold mt-3 first:mt-0" : "pl-2"
                                }`}
                              >
                                {trimmed}
                              </p>
      ...
      <IkSignaturesHTML permit={permit} />
    </div>
  );
};
